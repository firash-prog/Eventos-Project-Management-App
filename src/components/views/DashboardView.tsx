...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const budgetPercent = Math.round((totalSpent / totalAllocated) * 100);
    ...
    <div className="p-4 lg:p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        >
          ...
          <p className="text-[11px] text-slate-300 font-medium mt-2">Riyadh Gala & Auto Expo</p>
        </div>
        ...
        >
          ...
          <p className="text-[10px] text-slate-400 mt-1">SAR {(totalSpent / 1000000).toFixed(2)}M / {(totalAllocated / 1000000).toFixed(2)}M</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-5">
          ...
          <div className="text-3xl font-black text-slate-100">18</div>
          <p className="text-[11px] text-rose-400 font-medium mt-2">5 High Priority</p>
        </div>
        ...
        >
          ...
          <p className="text-[11px] text-amber-300 font-medium mt-2">1 Shift Conflict</p>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 border border-amber-500/30 text-amber-300 text-xs font-bold backdrop-blur-md">
              <Play className="w-3 h-3 fill-amber-300" /> NEXT FLAGSHIP PREMIERE
            </div>
            <h2 className="text-xl lg:text-2xl font-black text-slate-100 font-display">
              Riyadh Season Royal Gala 2026
            </h2>
            <p className="text-xs text-slate-300">
              Riyadh Front Arena • Expected Attendance: 4,500 VIP Guests • GEA Official Broadcast
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => {
                const gala = events.find((e) => e.id === 'evt-1');
                if (gala) onSelectEvent(gala);
              }}
